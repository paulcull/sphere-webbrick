export DRIVERNAME="sphere-webbrick"
export AUTHORNAME="Paul Cullender 'paulcull'"
export AUTHORWEBSITE="http://github.com/paulcull"
export SCRIPTVERSION="v0.1"
...
export GITHUBLINK="http://github.com/paulcull/$DRIVERNAME"
export SUPPORTLINK="http://github.com/paulcull/$DRIVERNAME"
...
function uPrompt { # Prompts the user for vaious information, such as hostname of the Sphere, username and password
  ...
  export NSIP=$DEFAULTHOST
  export NSUN=$DEFAULTUSERNAME
  export NSPW=$DEFAULTPASSWORD
}
...
#uPrompt
  ...
  echo "This script will install or update $DRIVERNAME on your Ninja Sphere. \nBefore you begin, please ensure you have SSH enabled on your Sphere. \nInformation on doing this can be found at https://developers.ninja/introduction/enable-ssh.html"
  echo "We're ready to install the driver.\n\nInstalling to: $NSIP\nUsername: $NSUN\nPassword: $NSPW\n\n Do you wish to continue?"
  echo "Installing.."
#else
...
if [ "$?" != "0" ] ; then
  echo "Unable to stop $DRIVERNAME on $NSIP. Please ensure the hostname is correct (try an IP address)\nand ensure your username and password are correct\n\nThis script will now exit"
...
if [ "$?" != "0" ] ; then
  echo "Unable to create folders on $NSIP. Please contact the author on $SUPPORTLINK for assistance"
...
if [ "$?" != "0" ] ; then
  echo "Unable to copy $DRIVERNAME to $NSIP. Please contact the author on $SUPPORTLINK for assistance" 
...
if [ "$?" != "0" ] ; then
  echo "Unable to copy package.json to $NSIP. Please contact the author on $SUPPORTLINK for assistance"
...
if [ "$?" != "0" ] ; then
  echo "Unable to start $DRIVERNAME on $NSIP. Please contact the author on $SUPPORTLINK for assistance"
